...
;(function($){
	...
	$.fn.shoppp = function(options) {
		...
		// Global options. All thats really needed here is the Email Address and the Currency Code. Currency Codes can be gotten from Paypal's site.
		...
		var settings = $.extend({}, $.fn.shoppp.opt, options);
		...
		return this.each(function() {
			...
			var product = $(this),
				itemTitle = product.find(".itemTitle").text(),
				itemDesc = product.find(".itemDesc").text(),
				itemPrice = product.find(".itemPrice").text(),
			...
			product.find('.buynow').attr("href", url + build);
